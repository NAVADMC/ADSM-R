...
ui <- fluidPage(
  ...
  titlePanel(title=div(img(src="USDA_ht100px.jpg"),"ADSM: Scenario Comparison")),
      ...
      sidebarLayout(
    sidebarPanel(
      checkboxGroupInput("scenario_variable", "Scenarios to compare:", c("Early","Late", "Vx Early","Vx Late")),
      tags$style(type="text/css", "textarea {width:100%}") ,
      tags$textarea(id="my_textarea", rows=10,placeholder =  "Comment...", value=""),
      
      # Button
      radioButtons('format', 'Document format', c('PDF', 'Word','HTML'),inline = TRUE),
    ...
    mainPanel( 
      ...
      tabsetPanel(
        ...
        tabPanel("Herd Level Analyses", 
                                  plotOutput('myplot7'), plotOutput('myplot8')
                 )
            )
        )
      )
