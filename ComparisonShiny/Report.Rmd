...
m =leaflet(MapMaster) %>% addTiles() %>%
      fitBounds(~min(Lon), ~min(Lat), ~max(Lon), ~max(Lat))
df = data.frame(filteredDataW)
m = leaflet(df) %>% addTiles()
m %>% addCircleMarkers(~Lon, ~Lat, popup=content, radius= ~ifelse(UnitSize,6,10), color = ~palInf(Type), stroke=F, fillOpacity = 0.1)
